...
import "github.com/l1nkkk/6.824/src/labrpc"
import "testing"
import "os"
...
import "sync"
import "runtime"
import "github.com/l1nkkk/6.824/src/raft"
import "fmt"
import "time"
import "sync/atomic"
...
// Maximum logger size across all servers
...
func (cfg *config) connectUnlocked(i int, to []int) {
	// logger.Printf("connect peer %d to %v\n", i, to)
...
func (cfg *config) disconnectUnlocked(i int, from []int) {
	// logger.Printf("disconnect peer %d from %v\n", i, from)
...
func (cfg *config) partition(p1 []int, p2 []int) {
	cfg.mu.Lock()
	defer cfg.mu.Unlock()
	// logger.Printf("partition servers into: %v %v\n", p1, p2)
	for i := 0; i < len(p1); i++ {
...
func (cfg *config) ConnectClientUnlocked(ck *Clerk, to []int) {
	// logger.Printf("ConnectClient %v to %v\n", ck, to)
...
func (cfg *config) DisconnectClientUnlocked(ck *Clerk, from []int) {
	// logger.Printf("DisconnectClient %v from %v\n", ck, from)
	endnames := cfg.clerks[ck]
	for j := 0; j < len(from); j++ {
		s := endnames[from[j]]

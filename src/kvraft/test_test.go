...
import "github.com/l1nkkk/6.824/src/porcupine"
import "github.com/l1nkkk/6.824/src/models"
import "testing"
import "strconv"
import "time"
import "math/rand"
...
func spawn_clients_and_wait(t *testing.T, cfg *config, ncli int, fn func(me int, ck *Clerk, t *testing.T)) {
	...
	// logger.Printf("spawn_clients_and_wait: waiting for clients")
	for cli := 0; cli < ncli; cli++ {
		ok := <-ca[cli]
		// logger.Printf("spawn_clients_and_wait: client %d is done\n", cli)
		if ok == false {
			t.Fatalf("failure")
		}
	}
...
// maxraftstate is a positive number, the size of the state for Raft (i.e., logger
...
// also checks that majority discards committed logger entries
...
func TestSnapshotSize3B(t *testing.T) {
	...
	// check that servers have thrown away most of their logger entries
	sz := cfg.LogSize()
	if sz > 8*maxraftstate {
		t.Fatalf("logs were not trimmed (%v > 8*%v)", sz, maxraftstate)
	}

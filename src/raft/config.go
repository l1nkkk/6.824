...
import "github.com/l1nkkk/6.824/src/labgob"
...
func (cfg *config) checkLogs(i int, m ApplyMsg) (string, bool) {
	...
	for j := 0; j < len(cfg.logs); j++ {
		if old, oldok := cfg.logs[j][m.CommandIndex]; oldok && old != v {
			log.Printf("%v: logger %v; server %v\n", i, cfg.logs[i], cfg.logs[j])
...
// applier reads message from apply ch and checks that they match the logger
// contents
...
// how many servers think a logger entry is committed?
